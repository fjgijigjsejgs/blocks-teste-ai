...
import { AppState, SimulationVoxel, RebuildTarget, VoxelData, SelectionMode } from '../types';
...
export class VoxelEngine {
  ...
  private selectionHighlightMesh: THREE.InstancedMesh | null = null;
  ...
  constructor(
    ...
    this.container.addEventListener('click', (e) => this.handlePointer(e));
  ...
  private handlePointer(event: MouseEvent) {
    if (this.selectionMode === SelectionMode.NONE || this.state !== AppState.STABLE) return;
    
    // Only handle clicks if the camera didn't move much (to avoid selection on drag)
    // For simplicity in this demo, we just use a standard click.
    ...
    if (intersects.length > 0) {
      ...
      if (instanceId !== undefined) {
        if (event.shiftKey) {
          if (this.selectedIndices.has(instanceId)) {
            this.selectedIndices.delete(instanceId);
          } else {
            this.selectedIndices.add(instanceId);
          }
        } else {
          const wasSelected = this.selectedIndices.has(instanceId) && this.selectedIndices.size === 1;
  ...
  private updateSelectionMesh() {
    if (!this.instanceMesh) return;

    if (this.selectionHighlightMesh) {
      this.scene.remove(this.selectionHighlightMesh);
      this.selectionHighlightMesh.geometry.dispose();
      (this.selectionHighlightMesh.material as THREE.Material).dispose();
    }
    ...
    const geo = new THREE.BoxGeometry(CONFIG.VOXEL_SIZE + 0.1, CONFIG.VOXEL_SIZE + 0.1, CONFIG.VOXEL_SIZE + 0.1);
    ...
    this.selectionHighlightMesh = new THREE.InstancedMesh(geo, mat, this.selectedIndices.size);
    let count = 0;
    this.selectedIndices.forEach(idx => {
      ...
      this.selectionHighlightMesh!.setMatrixAt(count++, this.dummy.matrix);
    });
    this.scene.add(this.selectionHighlightMesh);
  }
  ...
  public deleteSelected() {
    ...
    const newVoxels = this.voxels.filter((_, idx) => !this.selectedIndices.has(idx));
    const data: VoxelData[] = newVoxels.map(v => ({
      x: v.x, y: v.y, z: v.z, color: v.originalColor.getHex()
    }));
  ...
  public colorSelected(color: number) {
    if (this.selectedIndices.size === 0) return;
    const c = new THREE.Color(color);
    this.selectedIndices.forEach(idx => {
      this.voxels[idx].color.copy(c);
      this.voxels[idx].originalColor.copy(c);
    });
    this.draw();
  }
  ...
  private createVoxels(data: VoxelData[]) {
    ...
    this.voxels = data.map((v, i) => {
        ...
        return {
            id: i,
            x: v.x, y: v.y, z: v.z, color: c, originalColor: oc,
  ...
  private animate() {
    ...
    if (this.state !== AppState.STABLE || this.controls.autoRotate || this.selectedIndices.size > 0) {
        this.draw();
    }
