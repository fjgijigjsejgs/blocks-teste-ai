...
import { AppState, SavedModel, SelectionMode } from '../types';
import { Box, Bird, Cat, Rabbit, Users, Code2, Wand2, Hammer, FolderOpen, ChevronUp, FileJson, History, Play, Pause, Info, Wrench, Loader2, MousePointer2, Trash2, Palette } from 'lucide-react';
...
const PRESET_COLORS = [
  '#4A3728', '#F0F0F0', '#FFD700', '#111111', '#228B22', '#E5C100', '#FF5500', '#0077FF'
];
...
export const UIOverlay: React.FC<UIOverlayProps> = ({
  ...
  const isSelectionActive = selectionMode !== SelectionMode.NONE;
        ...
        <div className="pointer-events-auto flex flex-col items-end gap-2">
            <div className="flex gap-2">
                ...
                <TactileButton
                    ...
                    color={isSelectionActive ? 'amber' : 'slate'}
                    icon={<MousePointer2 size={18} strokeWidth={2.5} />}
                    label={isSelectionActive ? "Exit Selection" : "Select"}
                    compact
                />
                <TactileButton
                    onClick={onShowJson}
            ...
            {/* Selection Toolbar */}
            {isSelectionActive && (
              <div className="flex flex-col gap-2 p-3 bg-white/90 backdrop-blur-md rounded-2xl border-2 border-amber-200 shadow-xl animate-in slide-in-from-right-4 fade-in duration-300">
                  ...
                  <div className="flex gap-1.5 flex-wrap max-w-[140px]">
                    {PRESET_COLORS.map(color => (
                      <button
                        ...
                        className="w-5 h-5 rounded-md border border-slate-300 shadow-sm transition-transform active:scale-90 hover:scale-110"
                        style={{ backgroundColor: color }}
                      />
                    ))}
                  </div>
                )}
                <div className="text-[10px] text-slate-400 font-bold leading-tight mt-1 max-w-[140px]">
                  Click voxels to select. Shift+Click to multi-select.
                </div>
              </div>
            )}
        </div>
