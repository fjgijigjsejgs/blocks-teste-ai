...
import { AppState, VoxelData, SavedModel, SelectionMode } from './types';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const engine = new VoxelEngine(
      ...
      (indices) => setSelectedIndices(indices)
    );
  ...
  const handleToggleSelectionMode = () => {
    const newMode = selectionMode === SelectionMode.NONE ? SelectionMode.PICK : SelectionMode.NONE;
    setSelectionMode(newMode);
    engineRef.current?.setSelectionMode(newMode);
    if (newMode === SelectionMode.PICK) {
  ...
  const handleColorSelected = (colorHex: string) => {
    const colorInt = parseInt(colorHex.replace('#', ''), 16);
    engineRef.current?.colorSelected(colorInt);
  }
